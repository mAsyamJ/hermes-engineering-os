...
- `python -m engineering_os.experiments run-real` is wired and fail-closed
  ...
  the example JSON.
- `scripts/h2-present-budget.sh` / `scripts/h3-present-deploy.sh` present
  the next human gates. They do not authorize or deploy.
- Fail-closed production operators: `scripts/pag2-shadow.sh`,
  `scripts/pag2-canary.sh`, `scripts/pag2-rollback.sh`. Live CLI uses
  Unix IPC (`transport=ipc`); ubuntu is `BLOCKED_PEER` via SO_PEERCRED.
  hermes-op runtime probe: `scripts/pag2-as-runtime.sh pag2-probe`
...
- Secret-free backup helper: `scripts/pag2-backup.sh`. ubuntu writes
  `.runtime/pag2-backup/` only. hermes-op sudo writes
  `/var/backups/hermes-engineering-os/`. ubuntu sudo is refused. Isolated
  restore rehearsal: `scripts/pag2-restore-rehearsal.sh` (temp dir only; no live DB).
- Read-only gate dashboard: `scripts/pag2-status.sh`.
- H1 mechanical cutover `scripts/h1-cutover.sh` refuses ubuntu
...
- Persist auto-disable `scripts/pag2-rollback-persist.sh` is hermes-op

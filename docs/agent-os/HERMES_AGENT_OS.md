...
**Refresh:** `scripts/maintenance/capture-platform-overview.sh`, then `scripts/deployment/pag2-status.sh` and `scripts/verification/verify-operator-boundary.sh`
...
| Metric dictionaries / schemas | `docs/reference/METRIC_DICTIONARY.md`, `docs/*_DATABASE.md`, Gate X.1 matrices |
| Dated gate results | `docs/reports/phases/PHASE*_REPORT.md`, `docs/reports/production/PRODUCTION_READINESS_REPORT.md`, `docs/reports/pag/PAG*_REPORT.md` |
...
| `scripts/deployment/hermes-eos-deploy-tool.py` | Hash-locked install/rollback. ubuntu cannot apply |
| `bin/hermes-eos-verify` | Full acceptance including PAG-1/PAG-2 checks |
| `scripts/deployment/pag2-*.sh`, `scripts/deployment/h1-*.sh`, `scripts/deployment/h2-*.sh`, `scripts/deployment/h3-*.sh` | Human-gated operators |
...
- [docs/analytics/ANALYTICS_SOURCE_CAPABILITIES.md](../analytics/ANALYTICS_SOURCE_CAPABILITIES.md)
- [docs/evaluation/EVALUATION_CAPABILITY_MATRIX.md](../evaluation/EVALUATION_CAPABILITY_MATRIX.md)
- [docs/observability/PERFORMANCE_SOURCE_CAPABILITIES.md](../observability/PERFORMANCE_SOURCE_CAPABILITIES.md)
- [docs/experiments/EXPERIMENT_CAPABILITY_MATRIX.md](../experiments/EXPERIMENT_CAPABILITY_MATRIX.md)
- [docs/adaptation/ADAPTATION_CAPABILITY_MATRIX.md](../adaptation/ADAPTATION_CAPABILITY_MATRIX.md)
- [docs/reference/PLUGIN_VERIFICATION_MATRIX.md](../reference/PLUGIN_VERIFICATION_MATRIX.md)
...
**Today (not H1 PASS).** `ubuntu` still has `NOPASSWD: ALL`. Principals `hermes-op`, `hermes-runtime`, `hermes-actuator` **exist**, and some trust/unit files exist, but `scripts/verification/verify-operator-boundary.sh` returns **READY_FOR_HUMAN**. Reasons at stamp include `AUTH_AGENT_PASSWORDLESS_ROOT`, `AUTH_AGENT_IN_DOCKER_GROUP`, writable trust/unit paths, `AUTH_VERIFIER_CODE_WRITABLE`, `AUTH_GITHUB_ADMIN_ON_AGENT`, and `AUTH_GATEWAY_RUNS_AS_AGENT` (gateways are not running as `hermes-runtime`; they are masked).

PASS requires: ubuntu cannot gain unrestricted root; cannot replace verifier/trust/actuator; cannot impersonate the operator. Passwordless sudo makes PASS impossible. Human-only steps: [docs/reference/OPERATOR_BOOTSTRAP.md](../reference/OPERATOR_BOOTSTRAP.md). Architecture options: [docs/reference/OPERATOR_AUTHORITY_ARCHITECTURE.md](../reference/OPERATOR_AUTHORITY_ARCHITECTURE.md).
...
Accounts exist. Cutover (mask ubuntu gateways, start system units as `hermes-runtime`, reduce ubuntu sudo) is **not** done. Do not run `scripts/deployment/h1-cutover.sh` as ubuntu; it refuses that caller.
...
Dated write-ups: [PAG1_REPORT.md](../reports/pag/PAG1_REPORT.md), [PAG2_REPORT.md](../reports/pag/PAG2_REPORT.md), [PRODUCTION_READINESS_REPORT.md](../reports/production/PRODUCTION_READINESS_REPORT.md). Prefer `scripts/deployment/pag2-status.sh` over PIDs copied from August.

Human sequence after H1 PASS: `scripts/deployment/h2-present-budget.sh` → authorized `run-real` → `analyze-real` → H3 deploy-tool as `hermes-op` → Approval A → canary scripts. ubuntu must not self-authorize by inventing a budget file during autonomous work; a file may already exist from a human. This overview does not print that file.
...
Verify: `./bin/hermes-eos-verify`
Install plugin: `./scripts/deployment/install-plugin.sh` then dashboard-only restart
Never restart a gateway to “qualify” a doc.

Backups: `/var/backups/hermes-engineering-os/observability-*`. Restore only via isolated `scripts/observability/observability-db-verify.sh`, never onto live Postgres.
...
| `docs/reports/phases/PHASE1_REPORT.md` … `PHASE7_REPORT.md` | Framework at each close | dashboard PIDs, “not pushed”, GitHub BLOCKED_AUTH |
| `docs/reports/production/PRODUCTION_READINESS_REPORT.md` | PAR cells | “Do not push”; later commits exist |
| `docs/reports/pag/PAG1_REPORT.md` | PAG-1 EXAMPLE A | “Do not begin PAG-2”; PAG-2 tree exists; PIDs 924 / 2381797 |
| `docs/reports/pag/PAG2_REPORT.md` | PAG-2 autonomous work | header “not pushed”; origin is now at `432389f` |
| `docs/reports/pag/PAG2_PROPOSED_PLAN.md` | early plan | says not started |
| `docs/reports/phases/PHASE4–7_PROPOSED_PLAN.md` (if present) | early plans | contradict COMPLETE reports |

**Live source of truth:** capture script + `scripts/deployment/pag2-status.sh` + `scripts/verification/verify-operator-boundary.sh` + systemd/docker/HTTP. Historical reports stay as audit trail.
...
- `bin/hermes-eos-verify` remains the acceptance command.

Decision log (do not duplicate): [DECISIONS.md](../../DECISIONS.md). Threat model: [docs/operations/PRODUCTION_AUTHORITY_THREAT_MODEL.md](../operations/PRODUCTION_AUTHORITY_THREAT_MODEL.md).
...
Regenerate: `./scripts/maintenance/capture-platform-overview.sh`

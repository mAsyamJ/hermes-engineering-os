...
CLI: `scripts/experiments/experiment.sh` → compose profile `experiments`
...
artifact with `scripts/deployment/h2-write-authorization.sh` after the exact H2
...
`scripts/experiments/experiment-materialize.sh` every 5 minutes. Disable with

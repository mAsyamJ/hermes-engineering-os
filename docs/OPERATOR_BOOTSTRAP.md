...
satisfy. Read-only machine dashboard: `scripts/pag2-status.sh`.
...
- Action: install **public** Ed25519 trust only at
  ...
  actuator, hash-locked deploy-tool, `scripts/verify-operator-boundary.sh`,
  `scripts/pag2-inspect-ubuntu.sh`, and `deploy/pag2/eos-actuation-plugin/`
...
```bash
sudo /opt/hermes-engineering-os/scripts/h1-cutover.sh
```
...
- Verification: hermes-op still has sudo; ubuntu still logs in;
  `scripts/verify-operator-boundary.sh` status=`PASS`. The verifier
  ...
  `sudo scripts/pag2-as-runtime.sh pag2-probe`. Evidence-gated
...
- `scripts/h1-postcheck.sh` and `scripts/verify-operator-boundary.sh` both print PASS

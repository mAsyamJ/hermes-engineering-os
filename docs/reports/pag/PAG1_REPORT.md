...
| Item | Result |
|---|---|
| PAG1-0 entry freeze | PASS (`docs/reports/pag/PAG1_ENTRY_CHECK.md`, `tests/evidence/pag1-entry-20260829T065945Z.json`) |
...
| `scripts/verification/verify.sh` | PASS (179 unit tests; live Adaptation; `verify-pag1-data.sh` C pin MATCH `aff5125`) |
...
| Item | Result |
|---|---|
| Verifier | `scripts/verification/verify-operator-boundary.sh` (read-only) |
...
| Pin vs current main at report | MATCH (re-check at `scripts/verification/verify-pag1-data.sh` C) |
...
| PR package | `docs/operations/HERMES_UPSTREAM_PR_READY.md` |
...
| `docs/reports/pag/PAG1_CAUSAL_EVIDENCE.md` | BLOCKED_BUDGET |
...
|---|---|
| PAG-2 started | NO |
| Prerequisites | `docs/reports/pag/PAG2_PREREQUISITES.md` |
| Proposed later plan | `docs/reports/pag/PAG2_PROPOSED_PLAN.md` (not authorized) |
...
| Secret-like values in shipped docs/evidence | PASS (`scripts/verification/verify.sh` scan) |

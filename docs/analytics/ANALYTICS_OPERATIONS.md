...
Refresh: systemd user timer `hermes-eos-analytics.timer` every 5 minutes, oneshot `scripts/analytics/analytics-materialize.sh --json`.

Disable: `systemctl --user disable --now hermes-eos-analytics.timer`.

Manual: `scripts/analytics/analytics-materialize.sh --task <id> --json`.

Explain: `scripts/analytics/analytics-explain.sh <task_id> [board]`.

...
Backup: `scripts/observability/observability-db-backup.sh` dumps `phoenix`,
`hermes_engineering`, and `hermes_control` when present.

Isolated restore: `scripts/observability/observability-db-verify.sh` loads dumps into a

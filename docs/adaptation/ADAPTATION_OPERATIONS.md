...
CLI: `scripts/adaptation/adapt.sh <command>` (docker compose profile `adaptation`).
...
Timer: `hermes-eos-adaptation.timer` runs `scripts/adaptation/adaptation-materialize.sh`
with advisory lock `720260827`. It does not schedule Hermes tasks.

Emergency: `scripts/adaptation/adapt.sh disable-all` engages the kill switch file and

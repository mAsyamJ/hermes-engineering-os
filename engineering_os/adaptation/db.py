"""Control-plane Postgres helpers. Separate DSN from hermes_engineering."""
...
from engineering_os.adaptation import ADVISORY_LOCK_KEY


def database_url(role: str = "default") -> str:
    if role == "migrate":
        url = os.environ.get("CONTROL_MIGRATE_DSN") or os.environ.get("CONTROL_DATABASE_URL")
    elif role == "resolver":
        url = os.environ.get("CONTROL_RESOLVER_DATABASE_URL") or os.environ.get("CONTROL_DATABASE_URL")
    else:
        url = os.environ.get("CONTROL_DATABASE_URL")
    if not url:
        raise RuntimeError("CONTROL_DATABASE_URL is not set")
    return url


def analytics_url() -> str | None:
    return os.environ.get("ANALYTICS_DATABASE_URL")

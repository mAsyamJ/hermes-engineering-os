...
LOCKPID=$!
sleep 1
"$ROOT/scripts/performance-materialize.sh" --json >"$EVIDENCE/b-locked.json" || true
wait "$LOCKPID" || true
python3 - "$EVIDENCE" <<'PY'
import json, sys
from pathlib import Path
...
sudo -n docker exec hermes-eos-postgres \
  ...
  >"$EVIDENCE/g-lock.log" &
sleep 1
"$ROOT/scripts/performance-materialize.sh" --json >"$EVIDENCE/g-locked.json" || true
python3 - "$EVIDENCE" <<'PY'
import json, sys
from pathlib import Path
...
"$ROOT/scripts/verify-performance-data.sh" | tee "$EVIDENCE/j-verify.log"
assert_core
echo "PASS: J"

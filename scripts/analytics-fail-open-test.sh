...
"$ROOT/scripts/verify-analytics-data.sh" | tee "$EVIDENCE/d-quality.out"
...
SQL
set +e
"$ROOT/scripts/verify-analytics-data.sh" >"$EVIDENCE/f-quality-fail.out" 2>&1
FSTATUS=$?
set -e
test "$FSTATUS" -ne 0
...
SQL
"$ROOT/scripts/verify-analytics-data.sh" | tee "$EVIDENCE/f-quality-clean.out"

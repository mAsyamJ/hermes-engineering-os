...
# Robust against plugin nesting (agent_os/plugin or agent_os/integrations/hermes/plugin).
_PLUGIN_DIR = Path(__file__).resolve().parent


def _find_repo_root(start: Path) -> Path:
    for p in [start, *start.parents]:
        if (p / "plugin.yaml").is_file() and (p / "agent_os").is_dir() and (p / "pyproject.toml").is_file():
            return p
    raise RuntimeError(f"could not locate hermes-engineering-os repo root from {start}")


_REPO_ROOT = _find_repo_root(_PLUGIN_DIR)
...
def _tool_resolve_skill(args=None, **kwargs):
    ...
    from agent_os.core.router import route_task
...
def _tool_skill_search(args=None, **kwargs):
    ...
    from agent_os.core.resolver import hub_search, resolve_missing_capability
